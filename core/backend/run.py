...
def interactive_demo():
    ...
    while True:
        ...
        print("7. Exit")
        print("8. List all wallets")
        print("9. Show pending transactions")
        print("10. View latest block")
        print("11. Export blockchain to JSON")
        ...
        if choice == '1':
            ...
            if not blockchain.current_transactions:
                ...
                continue
            print(f"📄 Pending Transactions ({len(blockchain.current_transactions)}):")
            for i, tx in enumerate(blockchain.current_transactions, 1):
                print(f"  {i}. {tx['sender']} → {tx['recipient']}: {tx['amount']} coins (ID: {tx.get('transaction_id', 'N/A')})")
